...
async function getPlaylistItems(playlistId: string): Promise<Array<{ uri: string, uid: string }>> {
    try {
        const playlistUri = `spotify:playlist:${playlistId}`
        const contents = await Spicetify.Platform.PlaylistAPI.getContents(playlistUri)
        return (contents?.items ?? []).map((item: any) => ({ uri: item.uri, uid: item.uid })).filter((item: any) => item.uri)
    } catch (err) {
        console.error('[quick-add-to-playlist] Error fetching playlist contents:', err)
        return []
    }
}
...
async function getIsCurrentTrackInPlaylist(playlistId: string): Promise<boolean> {
    ...
    return items.some(item => item.uri === track?.uri)
}
...
async function main(): Promise<void> {
    ...
    const topbarButton: Spicetify.Topbar.Button = new Spicetify.Topbar.Button(
        ...
        async (self) => {
            ...
            if (self.icon === ICON_PLUS) {
                Spicetify.Platform.PlaylistAPI.add(playlistUri, [track.uri], { after: 'end' })
                    .then(() => {
                ...
                const match = items.find(item => item.uri === track.uri)
                if (!match || !match.uid) {
                    Spicetify.showNotification('Track not found in playlist!')
                    return
                }
                Spicetify.Platform.PlaylistAPI.remove(playlistUri, [{ uri: track.uri, uid: match.uid }])
                    .then(() => {
    ...
    new Spicetify.ContextMenu.Item(
        ...
        async ([uri], [uid] = [], _context = undefined) => {
            ...
            // Use Platform API to get playlist metadata (no HTTP request)
            ...
            updateSelectedStyles((oldPlaylist != null) ? oldPlaylist.id : null, playlistId)
            Spicetify.showNotification(`Selected playlist: "${metadata.name}"`)
    ...
    // @song-change: update Topbar icon (with debounce to avoid rapid updates)
    let songChangeTimeout: ReturnType<typeof setTimeout>
    Spicetify.Player.addEventListener('songchange', async () => {
        if (songChangeTimeout) clearTimeout(songChangeTimeout)
        topbarButton.icon = ICON_LOADING
        topbarButton.label = 'Loading...'
        songChangeTimeout = setTimeout(async () => {
            await updateTopbarButton(topbarButton)
        }, 500)
    })
