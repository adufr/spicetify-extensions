...
async function setupSettings (): Promise<SettingsSection> {
  ...
  settings.addButton('save', 'Apply changes now', 'Apply', () => {
    const styles = generateStyles(settings.getFieldValue('selected-bgcolor') as string)
    injectStyles(styles)
...
async function getIsCurrentTrackInPlaylist (playlistId: string): Promise<boolean> {
  ...
  let playlist = await Spicetify.CosmosAsync.get(`https://api.spotify.com/v1/playlists/${playlistId}/tracks?limit=100&fields=next,items(track(uri))`)
...
function generateStyles (selectedPlaylistBgColor: string = '#1c1818') {
  return `
...
async function main () {
  ...
  new Spicetify.ContextMenu.Item(
    ...
    async ([uri], [uid] = [], context = undefined) => {
      ...
      updateSelectedStyles(oldPlaylist? oldPlaylist.id : null, newPlaylist.id)
